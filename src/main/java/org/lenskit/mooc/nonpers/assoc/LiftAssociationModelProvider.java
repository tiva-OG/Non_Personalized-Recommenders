    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                float pXY = (float) yCount / (float) allUsers.size();
                float pX = (float) xUsers.size() / (float) allUsers.size();
                float pY = (float) yUsers.size() / (float) allUsers.size();

                float sXY = pXY / (pX * pY);
                itemScores.put(yId, sXY);
                yCount = 0;
            }
